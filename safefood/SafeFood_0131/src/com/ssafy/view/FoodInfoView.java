...
public class FoodInfoView {
	...
	private JButton[]		  title = new JButton[3];
	/**검색  조건*/
	private String	key;
	/**검색할 단어*/
	private String  word;
	...
	public void setMain(){
		...
		JPanel leftLN = new JPanel(new GridLayout(2, 1));
		...
		leftLN.add(imgL);
		leftLN.add(leftBottom);
		
		leftCenter.add(leftLN);
...
//		left.add(leftBottom,"South");
		...
		sortName = new JButton("이름정렬");
		rightTop.add(findC);
		rightTop.add(wordTf);
		rightTop.add(searchBt);
		rightTop.add(sortName);
		...
		sortName.addActionListener(buttonHandler);
